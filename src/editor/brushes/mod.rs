...
use drawing::{DrawingBrush, DrawingMode};
use selector::SelectorBrush;

// Different brush types
#[derive(Clone, Copy, PartialEq, Debug)]
pub enum BrushType {
    Drawing,  // Unified drawing brush (single tiles, platforms, stairs)
...
impl BrushManager {
    pub fn new() -> Self {
        Self {
            ...
            drawing_brush: DrawingBrush::new(),
    ...
    pub fn set_brush_type(&mut self, brush_type: BrushType) {
        ...
        self.drawing_brush.on_mouse_cancel(&mut Level::new(0, 0)); // Dummy level
    ...
    pub fn get_current_brush(&mut self) -> &mut dyn Brush {
        match self.brush_type {
            BrushType::Drawing => &mut self.drawing_brush,
    ...
    pub fn handle_mouse_release(&mut self, level: &mut Level) {
        let selected_tile = self.selected_tile.clone();
        let brush = self.get_current_brush();
        brush.on_mouse_release(level, selected_tile);
    }
    ...
    pub fn is_brush_active(&self) -> bool {
        match self.brush_type {
            BrushType::Drawing => self.drawing_brush.is_active(),
    ...
    pub fn set_drawing_mode(&mut self, mode: DrawingMode) { 
        self.drawing_brush.set_mode(mode); 
    }
    
    pub fn drawing_mode(&self) -> DrawingMode { 
        self.drawing_brush.mode() 
    }
} 